...
pub fn add(values: anytype) Op(values) {
    var op = Op(values){};
...
/// Accepts a tuple of integers
pub fn sub(values: anytype) Op(values) {
    var op = Op(values){};
    return inline for (0..values.len) |idx| {
        if (idx == 0) {
            op.result = values[idx];
...
pub fn Op(values: anytype) type {
    return struct {
        /// The value (with overflows)
        result: if (values.len >= 2) @TypeOf(values[0] +% values[1]) else @TypeOf(values[0]) = 0,
